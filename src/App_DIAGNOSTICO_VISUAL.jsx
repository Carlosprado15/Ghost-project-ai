...
export default function App() {
  ...
  // Estado do relógio - com smoothing cinematográfico leve
  const [wristCircle, setWristCircle] = useState({
    x: 0,
    y: 0,
    visible: false,
  });
  
  // Refs para smoothing leve
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log(
          '[DEBUG] Câmera iniciada - readyState:',
  ...
  // ✅ RELÓGIO GLB - melhorado visualmente
  const watchStyle = {
    position: 'fixed',
    left: `${wristCircle.x}px`,
    top: `${wristCircle.y}px`,
    width: '180px',  // AUMENTADO: 140px → 180px
    height: '180px', // AUMENTADO: 140px → 180px
    transform: 'translate(-50%, -50%) rotate(0deg)', // Rotação fixa por enquanto
    pointerEvents: 'none',
    zIndex: 15,
    opacity: wristCircle.visible ? 1 : 0,
    transition: 'opacity 0.2s ease',
    filter: 'drop-shadow(0 12px 32px rgba(0,0,0,0.7)) drop-shadow(0 4px 12px rgba(0,0,0,0.5))',
  };
    ...
    <div className="scanner">
      ...
      >
        ...
        <div style={{ color: '#0ff', marginBottom: 8 }}>
          Callbacks MediaPipe:{' '}
          <span style={{ color: '#fff' }}>{debugInfo.callbackCount}</span>
        </div>

        <div
          style={{
            color: debugInfo.hasLandmarks ? '#0f0' : '#f00',
            fontWeight: 'bold',
          }}
        >
        ...
        {debugInfo.hasLandmarks && (
          <>
            <div style={{ color: '#0ff' }}>
              Total Landmarks:{' '}
              <span style={{ color: '#fff' }}>{debugInfo.landmarkCount}</span>
            </div>
            ...
            >
              <div
                style={{ color: '#0f0', fontWeight: 'bold', marginBottom: 4 }}
              >
                🟢 WRIST (Landmark 0):
              </div>
              <div>
                Normalizado X:{' '}
                <span style={{ color: '#fff' }}>{debugInfo.wristNormX}</span>
              </div>
              <div>
                Normalizado Y:{' '}
                <span style={{ color: '#fff' }}>{debugInfo.wristNormY}</span>
              </div>
              <div style={{ marginTop: 4 }}>
                Tela X:{' '}
                <span style={{ color: '#fff' }}>{debugInfo.wristX}px</span>
              </div>
              <div>
                Tela Y:{' '}
                <span style={{ color: '#fff' }}>{debugInfo.wristY}px</span>
              </div>
            </div>
          </>
        )}
        ...
        <div style={{ marginTop: 10, fontSize: 10, color: '#888' }}>
          🟢 Círculo Verde: {wristCircle.visible ? 'VISÍVEL (60px)' : 'OCULTO'}
        </div>
        ...
        >
          ⚠️ Relógio DESABILITADO para diagnóstico
          <br />
          ⚠️ SEM smoothing - tracking puro
          <br />
      ...
      {!tracking && (
        <div className="tracking-hint">
          ...
          <p style={{ fontSize: '14px', marginTop: '8px' }}>
            (Modo diagnóstico - círculo verde 60px)
          </p>
        </div>
      )}
    </div>
