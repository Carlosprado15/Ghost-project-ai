...
const GLB_URL           = _ACTIVE_PRODUCT?.modelUrl ?? '/models/CW001.glb';
...
const AUTO_MESSAGES = {
  'waiting-hand': { text: 'APONTE PARA O PULSO',   sub: 'aguardando detecção…',     color: '#facc15' },
...
export default function TasksWristLab() {
  ...
  // ── Engine hook ─────────────────────────────────────────────────────────────
  const tracking = useGhostWristAR({ videoRef, enabled: camReady, onRawFrame });
  const { isTracking, position, rotationZ, scale, raw, filtered, fps, error: trackError, ready, mvReady, updateFilterPreset } = tracking;
  ...
  }, [tracking, activePreset, recState]);
  ...
  const renderAutoOverlay = () => {
    ...
    return (
      <div style={{
        ...
        background: isDone ? 'rgba(0,40,0,0.88)' : isFailed ? 'rgba(60,0,0,0.88)' : 'rgba(0,0,0,0.80)',
        display: 'flex', flexDirection: 'column',
        alignItems: 'center', justifyContent: 'center',
        padding: '24px 20px', textAlign: 'center',
      }}>
        {/* Main message */}
        <div style={{ color: msg.color, fontSize: '3.5rem', fontWeight: 900, lineHeight: 1.1, letterSpacing: 1, fontFamily: 'sans-serif' }}>
          {msg.text}
        </div>

        {/* Sub message */}
        {msg.sub && (
          <div style={{ color: '#e2e8f0', fontSize: '1.4rem', fontWeight: 600, marginTop: 16, fontFamily: 'sans-serif' }}>
            {msg.sub}
          </div>
        )}
        ...
        {/* Countdown */}
        {showCountdown && (
          <div style={{ color: '#fff', fontSize: '6rem', fontWeight: 900, marginTop: 20, fontFamily: 'sans-serif', lineHeight: 1 }}>
            {autoCountdown}
          </div>
        )}
        ...
        {/* Progress bar */}
        {showProgress && (
          <div style={{ width: '80%', maxWidth: 400, marginTop: 30 }}>
            <div style={{ background: 'rgba(255,255,255,0.15)', borderRadius: 8, height: 14, overflow: 'hidden' }}>
              <div style={{
            ...
            <div style={{ color: '#94a3b8', fontSize: '1rem', marginTop: 8, fontFamily: 'monospace' }}>
              {autoProgress}%
            </div>
          </div>
        )}
        ...
        {isDone && autoBestCombo && (
          <div style={{ marginTop: 28, fontFamily: 'monospace', fontSize: '1.1rem', color: '#e2e8f0', lineHeight: 1.8 }}>
            <div style={{ color: '#4ade80', fontSize: '1.3rem', fontWeight: 700 }}>Melhor combo aplicado:</div>
            <div>minCutoff = <span style={{ color: '#facc15' }}>{autoBestCombo.minCutoff}</span></div>
            <div>beta = <span style={{ color: '#facc15' }}>{autoBestCombo.beta}</span></div>
            <div>score = <span style={{ color: '#facc15' }}>{autoBestCombo.score.toFixed(3)}</span></div>
            <div style={{ color: '#94a3b8', fontSize: '0.9rem', marginTop: 12 }}>
              {mvReady ? 'Mova o pulso e avalie o relógio.' : 'Carregando model-viewer…'}
            </div>
            <div style={{ display: 'flex', gap: 12, marginTop: 20, justifyContent: 'center', flexWrap: 'wrap' }}>
              <button
                onClick={() => setGlbActive(v => !v)}
                style={{ ...mkBtn(glbActive ? '#16a34a' : '#7c3aed', false), fontSize: 15, padding: '12px 24px' }}
              >
                {glbActive ? '✓ GLB Ativo' : `Ativar GLB ${ACTIVE_PRODUCT_ID}`}
              </button>
              <button
                onClick={() => setAutoPhase(null)}
                style={{ ...mkBtn('#374151', false), fontSize: 15, padding: '12px 24px' }}
              >
                Ver HUD completo
              </button>
            </div>
        ...
        {/* Failed screen */}
        {isFailed && (
          <div style={{ marginTop: 20, color: '#fca5a5', fontSize: '1rem', fontFamily: 'sans-serif', maxWidth: 340, lineHeight: 1.6 }}>
            {autoError}
            <br /><br />
            <button
              onClick={() => {
              ...
              style={{ ...mkBtn('#dc2626', false), fontSize: 16, padding: '14px 28px' }}
            >
              Tentar novamente
            </button>
          </div>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000', overflow: 'hidden' }}>
      ...
      {/* GLB model-viewer — camada base, ancoragem proporcional ao pulso */}
      {glbActive && mvReady && isTracking && (
        <div style={{
          position:  'absolute',
      ...
      {/* ── HUD top-left (sempre visível quando não há overlay auto) ── */}
      {!autoPhase && (
        <div style={{ ...panelBase, position: 'absolute', top: 14, left: 14, zIndex: 10, minWidth: 210 }}>
          <div style={{ color: '#a78bfa', fontWeight: 700, fontSize: 13, marginBottom: 6 }}>
            Ghost Engine Lab — {ACTIVE_PRODUCT_ID}
          </div>
          {!ready    && !trackError && <div style={{ color: '#facc15' }}>Carregando HandLandmarker…</div>}
          {!mvReady  && <div style={{ color: '#94a3b8' }}>Carregando model-viewer…</div>}
          {trackError && <div style={{ color: '#f87171', wordBreak: 'break-word' }}>ERRO: {trackError}</div>}
          {camError   && <div style={{ color: '#f87171', wordBreak: 'break-word' }}>{camError}</div>}

          {[
            ['produto',  ACTIVE_PRODUCT_ID],
            ['câmera',   camLabel],
            ['status',   isTracking ? '✓ detectando' : '— aguardando'],
            ['fps',      fps],
            ['x filt',   filtered?.pos ? filtered.pos.x.toFixed(4) : '—'],
            ['y filt',   filtered?.pos ? filtered.pos.y.toFixed(4) : '—'],
            ['rotZ °',   filtered?.rotZ != null ? (filtered.rotZ * 180 / Math.PI).toFixed(1) + '°' : '—'],
            ['scale',    filtered?.scale?.toFixed(4) ?? '—'],
            ['jitter',   jitter],
            ['preset',   activePreset ? `mc=${activePreset.minCutoff} β=${activePreset.beta}` : 'padrão'],
            ['glb',      mvReady ? (glbActive ? 'ativo' : 'desligado') : 'carregando…'],
          ].map(([k, v]) => (
            <div key={k}>
              <span style={{ color: '#94a3b8' }}>{k}: </span>
              <span style={{ color: k === 'status' && isTracking ? '#4ade80' : k === 'status' ? '#f87171' : k === 'preset' && activePreset ? '#34d399' : '#93c5fd' }}>
                {String(v)}
              </span>
            </div>
          ))}
      ...
      {!autoPhase && (
        <div style={{ position: 'absolute', bottom: 20, left: '50%', transform: 'translateX(-50%)', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={() => setFacingMode(nextMode)} disabled={isRecording} style={mkBtn('#374151', isRecording)}>
              Câmera: {camLabel} →
            </button>
      ...
      {ready && !autoPhase && (
        <div style={{ position: 'absolute', top: 14, left: '50%', transform: 'translateX(-50%)', zIndex: 10, background: isTracking ? 'rgba(22,163,74,0.85)' : 'rgba(220,38,38,0.75)', color: '#fff', borderRadius: 20, padding: '5px 16px', fontSize: 13, fontFamily: 'monospace', fontWeight: 700 }}>
          {isTracking ? '● TRACKING' : '○ aguardando pulso'}
        </div>
      )}
    </div>
  );
