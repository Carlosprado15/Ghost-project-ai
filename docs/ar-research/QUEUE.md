...
Geradas por levantamento manual do Carlinhos (conversa paralela com Claude Chat, 02/09) + achado medido na Parte 2 do QA de catálogo (`qa-color-check.mjs`):

- QR-058 [P1][G][RESPONDIDA → AR-KB-011] `qa-color-check.mjs` (checagem de cor por distância RGB entre foto real e render 3D) tem um blind spot medido: contra o backup antigo/errado de CW033, não sinalizou divergência (distância 26.8, abaixo do limiar) mesmo o render sendo visivelmente um objeto errado/quebrado (blob branco sem padrão, ângulo errado) — porque o problema é IDENTIDADE/FORMA do objeto, não TOM de cor, e RGB médio não mede isso. Existe uma técnica de comparação por embedding visual (DINOv2 ou similar) que resolva isso, pra detectar o tipo de erro do CW006/CW007 (produto fisicamente diferente)? Registrado como HIPOTESE em AR-KB-011 — falta trazer as fontes reais da conversa com o Claude Chat pra confirmar/promover o veredito.
